...
parseFile fp = B.readFile fp >>= return . parseLines . B.lines

isStartYearLine = isDigit . B.head . B.dropWhile (== '-') . B.dropWhile isSpace
...
parseLines :: [B.ByteString] -> [DateTime]
parseLines ls = concatMap convert . go (error "parseLines") [] $ ls
  where go y acc (l:ls)
          | isStartYearLine l = let rest = go
                                           (read .
                                            B.unpack .
                                            B.takeWhile (\c -> isDigit c || c == '-') .
                                            B.dropWhile isSpace $ l)
                                           []
                                           ((B.replicate 7 ' ' `B.append` B.drop 7 l):ls)
                                in if acc == []
                                   then rest
                                   else ((y, acc):rest)
          | otherwise = let d = B.take 13 .  B.drop 8 $ l
                            acc' = if B.all isSpace d then acc
                                   else acc ++ [B.unpack d]
                        in go y acc' ls
        go y []  [] = []
        go y acc [] = [(y,acc)]
